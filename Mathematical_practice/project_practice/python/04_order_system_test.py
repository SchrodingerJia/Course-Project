...
def best_selling_dishes_analysis(dish_df, mode = -1):
    ...
    plt.tight_layout()
    plt.show()
def sales_trend_analysis(dish_df):
    """
    销售趋势分析
    """
    # 按月份统计
...
def price_analysis(dish_df, mode = 1):
    ...
    if mode == 1:
        ...
        # 消费价格分布
        sns.histplot(dish_stats['平均价格'], bins=20, kde=True, color='teal')
        plt.title('消费价格分布', fontsize=14, fontweight='bold')
        plt.xlabel('平均价格（元）', fontsize=12)
        plt.ylabel('消费次数', fontsize=12)
    plt.tight_layout()
    plt.show()
# 学生模式
def student(user,csv_file):
    ...
    while True:
        ...
        match choice:
            case '1':
                best_selling_dishes_analysis(dish_df,2)
                continue
            case '2':
                sales_trend_analysis(dish_df)
                continue
            case '3':
                merchant_analysis(dish_df,2)
                continue
            case '4':
                price_analysis(dish_df,2)
                continue
            case '0':
                print('退出系统')
                exit(0)
...
if __name__ == "__main__":
    student(('张三', 2), '../data/simulated_orders.csv')